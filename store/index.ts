...
export const actions: Actions<S, A, G, M> = {
  ...
  async fetchClipMovies (this: Vue, { commit }, { userId, page }) {
    try {
      this.$axios.setHeader('Authorization', localStorage.getItem('jwt') || false)
      const data = await this.$axios.$get(`${process.env.BaseUrl}/scrape/clip_movies`, { params: { userId, page } })
      return data
    } catch (e) {
      console.log(e)
    }
  ...
  async setClipMovies (this: Vue, { commit }, { movies }) {
    try {
      const data = await this.$axios.$post(`${process.env.BaseUrl}/movies`, { movies })
      commit('setMovies', { movies: data })
    } catch (e) {
      console.log(e)
    }
  ...
  async updateClipMoviesDB (this: Vue, { dispatch }, { movies }) {
    try {
      ...
      await dispatch('setClipMovies', { movies })
